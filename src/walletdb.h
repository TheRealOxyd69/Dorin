...
public:
    ...
    bool WriteZDORSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDORSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDORSeed_deprecated(uint256& seed);
    bool EraseZDORSeed();
    bool EraseZDORSeed_deprecated();

    bool WriteZDORCount(const uint32_t& nCount);
    bool ReadZDORCount(uint32_t& nCount);
