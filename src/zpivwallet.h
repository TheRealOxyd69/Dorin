...
#ifndef Dorin_ZDORWALLET_H
#define Dorin_ZDORWALLET_H
...
public:
    CzDORWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDOR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDOR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Dorin_ZDORWALLET_H
