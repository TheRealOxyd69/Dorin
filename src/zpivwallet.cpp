...
CzDORWallet::CzDORWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdor seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDORSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDORSeed_deprecated()) {
                    LogPrintf("%s: Updated zDOR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdor seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdor wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDORWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDORCount(nCountLastUsed);
    else if (!walletdb.ReadZDORCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDORWallet::Lock()
{
    seedMaster = 0;
}

void CzDORWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDORWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDOR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDORWallet::LoadMintPoolFromDB()
{
...
void CzDORWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDORWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDORWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdorTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDORWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDOR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdorTracker which also adds to database
    pwalletMain->zdorTracker->Add(dMint, true);
...
void CzDORWallet::SeedToZDOR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDORWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDORWallet::GenerateDeterministicZDOR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDORWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDORWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
