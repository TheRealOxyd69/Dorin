...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZDorStake(spend));
    } else {
        ...
        CDorStake* dorInput = new CDorStake();
        dorInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(dorInput);
    }
