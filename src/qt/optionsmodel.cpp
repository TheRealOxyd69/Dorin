...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DOR);
    ...
    if (!settings.contains("nAnonymizeDorinAmount"))
        settings.setValue("nAnonymizeDorinAmount", 1000);

    nAnonymizeDorinAmount = settings.value("nAnonymizeDorinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDorinAmount"))
        SoftSetArg("-anonymizedorinamount", settings.value("nAnonymizeDorinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dorin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDorinAmount:
            return QVariant(nAnonymizeDorinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDorinAmount:
            nAnonymizeDorinAmount = value.toInt();
            settings.setValue("nAnonymizeDorinAmount", nAnonymizeDorinAmount);
            emit anonymizeDorinAmountChanged(nAnonymizeDorinAmount);
            break;
        case CoinControlFeatures:
