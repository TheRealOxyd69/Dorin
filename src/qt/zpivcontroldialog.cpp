...
#include "ui_zdorcontroldialog.h"
...
std::set<std::string> ZDorControlDialog::setSelectedMints;
std::set<CMintMeta> ZDorControlDialog::setMints;

ZDorControlDialog::ZDorControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDorControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDorControlDialog::~ZDorControlDialog()
{
    delete ui;
}

void ZDorControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDorControlDialog::updateList()
{
...
void ZDorControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDorControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDor_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDorControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDorControlDialog::GetSelectedMints()
{
...
void ZDorControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
