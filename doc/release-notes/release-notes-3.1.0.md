Dorin Core version *3.1.0* is now available from:  <https://github.com/dorin-project/dorin/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/dorin-project/dorin/issues>


Mandatory Update
==============

Dorin Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zDOR Staking is here! zDOR staking will be activated on the morning of the 8th of May 2018. With the release of zDOR staking, there are effectively 2 versions of zDOR, zDOR minted on the 3.0.6 Dorin wallet or lower, and zDOR minted on Dorin wallet version or higher. New features in this release will require the use of zDOR v2, zDOR minted on this wallet release 3.1.0 or later. If you currently hold zDOR v1 and wish to take advantage of zDOR staking and deterministic zDOR, you will need to spend the zDOR v1 to yourself and remint zDOR v2.
Note: To find your zDOR version, click the privacy tab, then the zDOR Control button then expand the arrows next to the desired denomination.


### Deterministic zDOR Seed Keys

zDOR is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zDOR outside of the wallet that the zDOR had been minted on. zDOR can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zDOR minting

zDOR minting now only requires 1 further mint (down from 2) to mature. zDOR mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zDOR Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zDOR denomination has been spent or not.



DOR/zDOR Staking and Masternode Rewards
--------------

### DOR, zDOR and Masternode Payment Schedule

To encourage the use of zDOR and increase the Dorin zerocoin anonymity set, the Dorin payment schedule has been changed to the following:

If a user staking zDOR wins the reward for their block, the following zDOR reward will be: 
- 3 zDOR (3 x 1 denominations) rewarded to the staker, 2 DOR rewarded to the masternode owner and 1 DOR available for the budget. This is a total block reward of 6 DOR, up from 5.

If a user staking DOR wins the reward, the following amounts will be rewarded: 
- 2 DOR to the DOR staker, 3 DOR to the Masternode owner and 1 DOR available for the budget. This is a total block reward of 6 DOR, up from 5.


### Return change to sender when minting zDOR

Previously, zDOR minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most DOR and return the change from a zDOR mint to that address.
...
The visual layout of the Dorin Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zDOR minting feature. This is enabled by default and the enablezeromint=0 setting in the dorin.conf file will overwrite the GUI option.
-	The percentage of autominted zDOR can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular DOR.
...
### Summary
 ...
 The Dorin wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the dorin.conf file, found in the Dorin data directory.
- backuppath = <directory / full path>
- zdorbackuppath = <directory / full path>
...
* If zDOR backup, auto generated name is `wallet-autozdorbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
Dorin Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The Dorin client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running dorind without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Dorin support group, which often times sees users say "I didn't spend that zDOR". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Dorin masternodes, the monthly budget available to be utilised has been increased to 42,000 DOR / month. This DOR only has the opportunity to be raised once per month (paid to winning proposals) with any unused DOR not created by the blockchain.

### Budget Finalization Fee

The Dorin finalization fee for successful proposals has now been reduced, this fee is now 5 DOR down from 50 DOR. The total fee outlay for a successful proposal is now a total of 55 DOR.
...
-	Enforced v1 zDOR spends to require a security level of 100
-	Updates to zDOR spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the Dorin-Qt masternodes tab
-	Updated Dorin-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and Dorin's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zDOR Control dialog in the QT wallet, a user is able to see zDOR marked as version 1 or 2.

zPoS (zDOR staking)
--------------

Once a zDOR has over 200 confirmations it becomes available to stake. Staking zDOR will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zDOR. So for example if a 1,000 zDOR denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zDORs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within Dorin's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zDOR, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zDOR that can be recalculated at any time using the seed. Deterministic zDOR allows for users to backup all of their future zDOR by simply recording their seed and keeping it in a safe place (similar to backing up a private key for DOR). The zDOR seed needs to remain in the wallet in order to spend the zDOR after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zDOR data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

Dorin's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Dorin used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Dorin using the bug bounty program. The modulus is now correctly imported and Dorin's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zDOR v2: zPoS dzDOR ezDOR external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zDOR balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zDOR status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zDor Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault dorin-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zDOR spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zDOR. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zDOR. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/dorin-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
